...
test |> leaflet() |> addProviderTiles("OpenStreetMap") |>
   addMarkers(icon = ~ firesmall, popup = ~htmlEscape(acres))

test <- test |> 
  mutate(acrespopup = str_glue("Fire Size: {acres} Acres"))

test |> leaflet() |> addProviderTiles("OpenStreetMap") |>
   addMarkers(icon = ~ firesmall,  popup = ~htmlEscape(acrespopup))
